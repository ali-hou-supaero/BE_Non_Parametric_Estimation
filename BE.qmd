...
```
:::
Based on the graph above we see that the optimal bandwidth is $\approx 0.015$ because it is the value that minimizes LOO risk.
...
```
The time complexity of the LOOCV function looks polynomial. We won't be able to tu use this estimator on large sample sizes. We are confronted here by the curse of dimensionality seen in class.
:::
## LOOCV, take 2
...
The time complexity of LOOCV2 is better than LOOCV1. Its time complexity looks linear compared to LOOCV1 making it more suitable for large values of $n$.
...
```
:::
We observe that LOOCV3 is a bit better than LOOCV1 however, both appear to follow the same polynomial trend. It is clear from the plot that the LOOCV2 implementation performs best for large values of $n$.
...
::: {.callout-note collapse="false"}
### Answer `r QQ`
#### Proof of the LP1 Estimator Formula

We aim to derive the expression for the local polynomial estimator of degree 1 (LP1) at a point $x$, using kernel-weighted least squares.

We fit a local linear model of the form:

$$
f(z) \approx a_0 + a_1(z - x)
$$

by minimizing the weighted sum of squares:

$$
\sum_{i=1}^n \left( y_i - a_0 - a_1(x_i - x) \right)^2 K\left( \frac{x_i - x}{h} \right)
$$

This is a weighted least squares problem with design matrix:
...
1 & x_n - x
\end{bmatrix}, \quad
W = \text{diag}\left( K\left( \frac{x_1 - x}{h} \right), \dots, K\left( \frac{x_n - x}{h} \right) \right), \quad
Y = \begin{bmatrix}
y_1 \\ y_2 \\ \vdots \\ y_n
\end{bmatrix}
$$
...
Let us define the following weighted sums:

$$
S_k(x) = \sum_{i=1}^n (x_i - x)^k K\left( \frac{x_i - x}{h} \right), \quad
S_k'(x) = \sum_{i=1}^n (x_i - x)^k y_i K\left( \frac{x_i - x}{h} \right)
$$
...
The LP1 estimator at $x$ is the intercept $\hat{f}^{LP1}(x) = \hat{\beta}_0$, so:
