...
::: {.callout-note collapse="false"}
### Answer `r QQ`
The time complexity of the LOOCV function looks polynomial. We won't be able to tu use this estimator on large sample sizes. We are not confronted here to the curse of dimensionality seen in class (related to the statistical complexity), since we are raising the number of samples and not the dimension. But the time complexity is quadratic. 
...
opt_bw_1 = bandwidths[which.min(res1)]
opt_bw_2 = bandwidths[which.min(res2)]
cat("Optimal bandwidth LOOCV1:", opt_bw_1, "\n")
cat("Optimal bandwidth LOOCV2:", opt_bw_2, "\n")
...
legend("topright", legend = c("LOOCV1 (locpoly)", "LOOCV2 (Math def)"),
       col = c("blue", "red"), lty = 1, pch = c(1, 19))
```
We observe a consistent offset between the two LOOCV risk curves across all bandwidths, indicating that the methods are not perfectly equivalent in their numerical behavior. However, since the optimal bandwidth is not affected too much by these differences, we can conclude that both implementations are correct and yield comparable results.
:::
...
plot(n_values, times_LOOCV, type = "b", col = "red", lwd = 2,
     ...
     main = "Run time LOOCV1 vs LOOCV2", 
     )
lines(n_values, times_LOOCV2, col = "blue", lwd = 2, type = "b")
legend("topleft", legend = c("LOOCV1 (locpoly)", "LOOCV2 (math def)"),
...
::: {.callout-note collapse="false"}
### Answer `r QQ`
By definition, the leave-one-out (LOO) cross-validation risk is:  
$$
LOOCV(h) = \frac{1}{n} \sum_{i=1}^n \left(Y_i - \hat{f}^{-i}_{h}(X_i) \right)^2
$$
...
$$
\hat{f}^{-i}_{h}(x) = \sum^n_{j=1, j\neq i}Y_j W_{nj}(x,h) = \frac{\sum^n_{j=1, j\neq i}Y_j K\left(\frac{X_j-x}{h}\right)}{\sum^n_{j=1, j\neq i} K\left(\frac{X_j-x}{h}\right)}
$$  
$$
\hat{f}^{-i}_{h}(x) = \frac{\sum^n_{j=1}Y_j K\left(\frac{X_j-x}{h}\right) - Y_i K\left(\frac{X_i-x}{h}\right)}{\sum^n_{j=1} K\left(\frac{X_j-x}{h}\right) - K\left(\frac{X_i-x}{h}\right)}
$$

Let's note $S = \sum^n_{i=1}K\left(\frac{X_i-x}{h}\right)$. Thus, we can write:  
$$
\hat{f}^{-i}_{h}(x) = \frac{S \cdot \hat{f}_{h}(x) - Y_i K\left(\frac{X_i-x}{h}\right)}{S - K\left(\frac{X_i-x}{h}\right)}
$$

From this point, if we isolate $\hat{f}_h$, we have:  
$$
\hat{f}_h(x) = Y_i \cdot \frac{K\left(\frac{X_i - x}{h}\right)}{S} + \hat{f}^{-i}_h(x)\cdot\frac{S-K\left(\frac{X_i - x}{h}\right)}{S}
$$

But we can also write $\frac{K\left(\frac{X_i - x}{h}\right)}{S}$ as $W_{ni}(x,h)$. So:  
$$
\hat{f}_h(x) = Y_i \cdot W_{ni}(x,h) + \hat{f}^{-i}_h(x)\cdot(1-W_{ni}(x,h))
$$

Let's isolate $\hat{f}^{-i}_h$:  
$$
\hat{f}^{-i}_h = \frac{\hat{f}_h - Y_i \cdot W_{ni}}{1-W_{ni}}
$$

We can now integrate this expression in the expression of the LOOCV risk:  
$$
...
opt_bw_1 = bandwidths[which.min(res1)]
opt_bw_2 = bandwidths[which.min(res2)]
opt_bw_3 = bandwidths[which.min(res3)]
cat("Optimal bandwidth LOOCV1:", opt_bw_1, "\n")
cat("Optimal bandwidth LOOCV2:", opt_bw_2, "\n")
cat("Optimal bandwidth LOOCV3:", opt_bw_3, "\n")
...
```
The graph indicates that the three implementations yield similar results. LOOCV2 and LOOCV3 match perfectly. However, a small discrepancy remains with the LOOCV1 implementation. Since this difference is minimal and does not affect by much the value of the optimal bandwidth, we consider the three implementations equivalent in terms of accuracy. Therefore, we can focus on comparing their computational efficiency
:::
...
legend("topleft", legend = c("LOOCV1 (locpoly)", "LOOCV2 (kernel)", "LOOCV3 (weights)"),
...
$$
\hat{\theta}_n(x) = \arg \min_{\theta \in \mathbb{R}^2} \sum^n_{i=1}\left( Y_i -  \theta^\top \Pi_1\left(\frac{X_i -x}{h}\right)\right)^2 K\left(\frac{X_i-x}{h}\right)
$$

with $\Pi_1(x) = a_0 + a_1\cdot x$. And we can redefine this problem as a weighted least square problem with the solution:  
$$
...
Now, thanks to the definitions of $S_k$ for $k \in [0,1,2]$, we have:  
$$
X^\top W X =
\begin{bmatrix}
S_0(x) & S_1(x) \\
...
We can inverse this $2 \times 2$ matrix:  
$$
(X^\top W X)^{-1} =
...
# Print optimal bandwidths
opt_bw_1 = bandwidths[which.min(res_lp1_slow)]
opt_bw_2 = bandwidths[which.min(res_lp1_fast)]
cat("Optimal bandwidth LOOCV1_LP1:", opt_bw_1, "\n")
cat("Optimal bandwidth LOOCV2_LP1:", opt_bw_2, "\n")


plot(bandwidths, res_lp1_slow, type = "b", col = "blue", ylim = range(c(res_lp1_slow, res_lp1_fast)),
     ylab = "LOO Risk", xlab = "Bandwidth", main = "LOOCV1_LP1 vs LOOCV2_LP1")
lines(bandwidths, res_lp1_fast, type = "b", col = "darkgreen", pch = 19)
legend("topright", legend = c("LOOCV1_LP1 (locpoly)", "LOOCV2_LP1 (equation (3))"),
...
optimal_bw_L1 <- bandwidths[which.min(res_lp1_slow)]
cat("Optimal bandwidth for LOOCV1_LP1:", optimal_bw_L1, "\n")

# Runtime comparison
# We reduce the number of point so that compilation doesn't take too long
n_values_small <- seq(from = 100, to = 1500, by = 100)

# Measure time for LOOCV_LP1
times_lp1_slow <- sapply(n_values_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
...
times_lp1_fast <- sapply(n_values_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
...
plot(n_values_small, times_lp1_slow, type = "b", col = "blue", lwd = 2,
     xlab = "Sample size (n)", ylab = "Time (s)",
     main = "Runtime: LOOCV1_LP1 vs LOOCV2_LP1")
lines(n_values_small, times_lp1_fast, type = "b", col = "darkgreen", lwd = 2, pch = 19)
legend("topleft", legend = c("LOOCV1_LP1 (locpoly)", "LOOCV2_LP1 (equation (3))"),
...
To extract the weights $W_{ni}(X_j)$, we evaluate the estimator at each $X_j$ using a vector $Y_i$ that is zero everywhere except at position $i$, where it equals 1.

In the code:

- For each $i = 1, \dots, n$, we construct a vector $Y_i$ with a 1 at position $i$ and 0 elsewhere.
- We then apply `locpoly(X, Y_i, ...)`, which returns the fitted values at all grid points.
...
cat("Optimal bandwidth LOOCV1_LP1:", opt_bw_1, "\n")
cat("Optimal bandwidth LOOCV2_LP1:", opt_bw_2, "\n")
cat("Optimal bandwidth LOOCV3_LP1:", opt_bw_3, "\n")
...
# Runtime comparison
# We reduce the number of point so that compilation doesn't take too long
n_values_very_small <- seq(from = 100, to = 1000, by = 100)

# Measure time for LOOCV_LP1
times_lp1_1 <- sapply(n_values_very_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
  system.time(LOOCV_LP1(X, Y, optimal_bw_L1))["elapsed"]
})
...
# Measure time for LOOCV2_LP1
times_lp1_2 <- sapply(n_values_very_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
  system.time(LOOCV2_LP1(X, Y, optimal_bw_L1))["elapsed"]
})
...
times_lp1_3 <- sapply(n_values_very_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
...
# Plot runtime comparison
plot(n_values_very_small, times_lp1_1, type = "b", col = "blue", lwd = 2,
     xlab = "Sample size (n)", ylab = "Time (s)",
     main = "Runtime: LOOCV_LP1 vs LOOCV2_LP1 vs LOOCV3_LP1")
lines(n_values_very_small, times_lp1_2, type = "b", col = "red", lwd = 2, pch = 17)
lines(n_values_very_small, times_lp1_3, type = "b", col = "darkgreen", lwd = 2, pch = 19)
...
# Print optimal bandwidths
opt_bw_1 = bandwidths[which.min(risk_loocv1)]
opt_bw_2 = bandwidths[which.min(risk_loocv2)]
opt_bw_3 = bandwidths[which.min(risk_kcv)]
cat("Optimal bandwidth LOOCV1_LP1:", opt_bw_1, "\n")
cat("Optimal bandwidth LOOCV2_LP1:", opt_bw_2, "\n")
cat("Optimal bandwidth K-Fold:", opt_bw_3, "\n")
...
# Runtime comparison
# Measure time for LOOCV_LP1
times_loocv1 <- sapply(n_values_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
  system.time(LOOCV_LP1(X, Y, optimal_bw_L1))["elapsed"]
})
...
# Measure time for LOOCV2_LP1
times_loocv2 <- sapply(n_values_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
  system.time(LOOCV2_LP1(X, Y, optimal_bw_L1))["elapsed"]
})
...
times_kfold <- sapply(n_values_small, function(n) {
  X <- 1:n / n
  Y <- sim_Y(X)
...
# Plot runtime comparison
plot(n_values_small, times_loocv1, type = "b", col = "blue", lwd = 2,
     xlab = "Sample size (n)", ylab = "Time (s)",
     main = "Runtime: LOOCV vs K-Fold CV (LP1)")
lines(n_values_small, times_loocv2, type = "b", col = "red", lwd = 2, pch = 17)
lines(n_values_small, times_kfold, type = "b", col = "darkgreen", lwd = 2, pch = 19)
...
```
With the example of 5-fold cross-validation, we see that computation time is slightly shorter than with LOOCV2_LP1. This makes it a more practical method for selecting the optimal bandwidth in real-world contexts, where sample sizes can become much larger than those simulated here.
:::
